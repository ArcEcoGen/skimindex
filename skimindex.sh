...
#   --mount SRC:DST      Bind-mount SRC (host) to DST (container).
#                        May be repeated for multiple extra mounts.
#   --dev                Same as shell --dev.
#   SCRIPT [ARGS...]     Path to the bash script to run (and its arguments).
#
# Configuration:
...
# _ski_run_script extra_mounts_count script [args...]
#   Runs a host bash script inside the container with the same mounts as
#   _ski_run_interactive, but non-interactively.
#   $1        = number of extra mounts that follow
#   $2..N     = extra mount specs
#   remaining = script path + args
_ski_run_script() {
    local _nmounts="$1"; shift
    local _extra_mounts=()
    for (( _i=0; _i<_nmounts; _i++ )); do
        _extra_mounts+=("$1"); shift
    done
    ...
    if [[ "$RUNTIME" == "apptainer" ]]; then
        _ski_build_bind_array "--bind"
        for _m in "${_extra_mounts[@]}"; do BIND+=(--bind "$(_ski_abs_mount "$_m")"); done
        BIND+=(--bind "${_script_abs}:/tmp/__ski_script.sh:ro")
        [[ -n "$_SKI_CONFIG_OVERRIDE" ]] && BIND+=(--bind "${_SKI_CONFIG_OVERRIDE}:/config/skimindex.toml")
        APPTAINERENV_PREPEND_PATH=/app/bin:/app/scripts \
        apptainer exec --pwd /app "${BIND[@]}" "$SIF_FILE" \
            bash --init-file "$_shell_init" /tmp/__ski_script.sh "$@"
    else
        _ski_build_bind_array "-v"
        for _m in "${_extra_mounts[@]}"; do BIND+=(-v "$(_ski_abs_mount "$_m")"); done
        BIND+=(-v "${_script_abs}:/tmp/__ski_script.sh:ro")
        [[ -n "$_SKI_CONFIG_OVERRIDE" ]] && BIND+=(-v "${_SKI_CONFIG_OVERRIDE}:/config/skimindex.toml:ro")
        ...
        while [[ $# -gt 0 ]]; do
            case "$1" in
                -h|--help)
                    ...
                    exit 0 ;;
                --dev)
                    _script_mounts+=("${PROJECT_ROOT}/obiluascripts:/app/obiluascripts")
                    shift ;;
                --mount)     _script_mounts+=("$2"); shift 2 ;;
                --mount=*)   _script_mounts+=("${1#--mount=}"); shift ;;
                --)          shift; break ;;
                -*)          logerror "Unknown script option: $1"; exit 1 ;;
                *)           break ;;
            esac
        done
        if [[ $# -eq 0 ]]; then
            logerror "script: missing script path."
            logerror "Run '$(basename "$0") script --help' for usage."
            exit 1
        fi
        _ski_run_script "${#_script_mounts[@]}" "${_script_mounts[@]+"${_script_mounts[@]}"}" "$@"
        exit 0
        ;;
esac
