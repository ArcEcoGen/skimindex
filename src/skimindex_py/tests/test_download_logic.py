...
class TestRefgenomePureLogic:
    def test_safe_name_replaces_spaces(self):
        from skimindex.download.refgenome import _safe_name
    ...
    def test_get_accession_type_gcf_is_zero(self):
        from skimindex.download.refgenome import _get_accession_type
    ...
    def test_get_genome_size_parses_int(self):
        from skimindex.download.refgenome import _get_genome_size
    ...
    def test_filter_by_species_one_per_species(self):
        from skimindex.download.refgenome import filter_assemblies_by_species
        assemblies = [
    ...
    def test_filter_by_genus_one_per_genus(self):
        from skimindex.download.refgenome import filter_assemblies_by_genus
        assemblies = [
            self._make_assembly("GCF_001", "Homo sapiens", 3_000_000_000),
...
class TestGenbankPureLogic:
    def test_get_ftp_listing_parses_bct_files(self):
        from skimindex.download.genbank import get_ftp_listing
        fake_output = (
        ...
        with patch("skimindex.download.genbank.curl_download") as mock_curl:
    ...
    def test_list_divisions_from_config(self):
        ...
        mock_cfg = MagicMock()
        mock_cfg.get.return_value = "bct pln vrl"
        with patch("skimindex.download.genbank.config", return_value=mock_cfg):
            result = list_divisions()
        assert result == "bct,pln,vrl"

    def test_get_release_number_strips_whitespace(self):
        from skimindex.download import genbank
